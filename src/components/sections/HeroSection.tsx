  ...
  return (
    <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-6 flex flex-col items-center">
        ...
        {/* Year */}
        <div className="font-bebas text-[64px] leading-none text-white mb-1">{t("year")}</div>
      ...
      <div className="absolute bottom-10 inset-x-0 px-10 z-20 flex items-end justify-between max-w-[1440px] mx-auto w-full">
        <div>
          <p className="font-inter text-[10px] font-bold tracking-[0.25em] text-white/40 uppercase mb-3">{t("journey")}</p>
          <div className="flex gap-10">
            {[["245", t("days")], ["14", t("hours")], ["32", t("minutes")]].map(([v, l]) => (
        ...
        <div className="text-right hidden md:block">
          <p className="font-inter text-[10px] font-bold tracking-[0.25em] text-white/40 uppercase mb-1">{t("location_label")}</p>
          <p className="font-bebas text-[28px] text-white tracking-widest">{t("location")}</p>
        </div>
      </div>
    </section>
  )
